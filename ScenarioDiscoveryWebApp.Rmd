...
### Part 1: Select policy, uncertainty metrics, and objectives to plot {data-height=200}
...
fluidRow(column(width=9, #fluidRow for dropdown button
           ...
           dropdownButton(circle = F, label = "Choose uncertainty metrics",width = "150%",inputId = "UMdropdown",

fluidRow(column(width=10,
            selectInput(inputId = "Window", label = "Select rolling window length (years)",choices = c("2", "5", "8", "10", "20", "44", "any"), selected = "10")
                ),
...
h5("Include duration and frequency metrics?"),
...
renderUI({
  req(input$DurFreq==TRUE)
    ...
    renderUI({selectInput("PlotMetrics",label="Select uncertainty metrics to plot", multiple = T, choices=colnames(x$x))})
...
selectInput("PlotObj",label="Select objectives to plot", multiple = T, choices=colnames(obj_all)[-c(1,2)]),
...
# bsTooltip(id = "UMdropdown", title = "Select uncertainty metrics for use in the Scenario Discovery algorithm and for plotting in the scatter matrix to the right. See the tooltips and information buttons for each user entry for more.", placement = "right", options=list(container="body") )
...
```{r, uncertainty metrics pairs plot}
...
renderPlot({
  ...
  validate(need(input$PlotMetrics, "Select uncertainty metrics."))
...
```{r, absolute panel Uncertainty metrics}
...
absolutePanel(class = "panel panel-default", fixed = T,
   ...
   tags$div(id="buttonUM", align="center",
            ...
            #   req(y$COI)
            # 
            #   actionButton(inputId = "gotoSD1",label = "Go to Scenario Discovery", onclick="location.href='#section-perform-scenario-discovery';", width = "180px")
            #   
            #   # actionButton(inputId = "gotoSD1",label = "Go to Scenario Discovery", onclick="location.href='#section-b-perform-scenario-discovery';", width = "180px")
            #     
            # }),
...
#   div(style="margin-bottom:20px"),
#   
#   actionButton(inputId = "SDButton",label = "Perform scenario discovery", width = "230px")             
#                 
#                 ),
...
#       tags$div(id = 'boxPT', class="collapse in", align="center",
# 
#         renderText(paste0(y$nCOI, " / ", nrow(x$x)))),
# 
#    HTML('<button data-toggle="collapse" data-target="#boxPT2" class="btn-block btn-primary">Objective: threshold</button>'),
#    tags$div(id = 'boxPT2', class="collapse in",
# 
...
# bsTooltip(id = "OpenModalSD", placement = "right", title="select Scenario Discovery options",options=list(container="body"))

# bsModal(id = "ModalSD", title = "Select PRIM inputs and algorithm",trigger = "OpenModalSD",
#         
#         h5("Choose PRIM algorithm type:"),
# prettyToggle(inputId = "PRIMType",label_on = "normal PRIM",label_off = "multi-objective PRIM", value = FALSE),
#         
...
# observeEvent({list(input$policyID, input$COIButton, input$Window, input$PRIMType, input$SOWensemble)}, {
#   
#   showNotification("Inputs have changed. Please run scenario discovery.", duration = NULL, type = "warning")
#   
# }, ignoreInit = T)

```

```{r, absolute panel on PT page}

# absolutePanel(class = "panel panel-default", fixed = T,
#                 draggable = TRUE, top = 62, left = 318, right ="auto", bottom = "auto",
#                 width = '13%', height = 'auto',
#                 style = "overflow-y:scroll; max-height: 142px; opacity: 0.9; style = z-index: 400",
# 
#               h4(strong("Vulnerability statistics")),
# 
#       HTML('<button data-toggle="collapse" data-target="#box1" class="btn-block btn-primary">number of vulnerable SOW</button>'),
#       tags$div(id = 'box1', class="collapse in", align="center",
# 
#         renderText(paste0(y$nCOI, " / ", nrow(x$x)))),
# 
#    HTML('<button data-toggle="collapse" data-target="#box2" class="btn-block btn-primary">Objective: threshold</button>'),
...
SOW_all=dplyr::select(SOW_all, -matches("RolDef."))
...
observeEvent(input$SOWensemble, {
  ...
  x$x=dplyr::select(x$x, -matches("RolDef."))
...
```{r, filter uncertainty metrics by UI}
...
observeEvent(input$Window,{ 
  ...
  always_keep=c("Demand", "Mead.PE", "Powell.PE", "min", "max")
  ...
  if(yr=="anyyr"){
    ...
    cols=c(always_keep, "mean", "median")
...
observeEvent(input$SDButton, {
  ...
  if(input$PRIMType==FALSE){ # MO PRIM
      ...
      box$Data=MO.PRIM(x=x$x, y=y$COI, qp_val=input$QPval, peel_crit = 2)


  } else {
    
    temp=prim(x=x$x, y=y$COI, peel_crit = 2, remove_dims = T)
...
# boxData=multiobjective_prim(x=x, y=y, qp_val=.12)
...
# dimensions=vector()
# for (i in 1:length(boxData$dimlist)){
#   dimensions[i]=sum(boxData$dimlist[[i]]$either)
# }

# data=data.frame(coverage=boxData$relcoverage, density=boxData$y.mean, dimensions, id=1:length(boxData$x))
...
output$dynamic=renderPlotly({
  ...
  # dynamic=layout(p=dynamic, legend=list(title=list(font=list(size=10), side="left")))
  # 
  # dynamic
...
observeEvent(eventExpr = event_data("plotly_click", source = "PT"),{
  ...
  # box$ID=temp$pointNumber+1 # plus one because plotly.js is zero indexed, R is 1 indexed
...
validate(need(x$x, "Select uncertainty metrics on the Define vulnerability page."))
...
#      
#   renderUI({
#   selectInput(inputId = "saved1",label = h4(strong("Choose scenario to display")), choices = c("Select analysis", names(database)), selected = NULL, width = "350px")
# })  
#     )
# 
#       )
...
fluidRow(column(width = 10, offset = 1,
  ...
  # splitLayout(cellWidths = c("20%","70%", "10%"),h5(""),h5(strong("Lake Mead operation diagram")), circleButton(inputId = "HideMead", icon=icon("caret-down"), size = "xs"))
...
#HTML('<button data-toggle="collapse"  data-target="#DV1" " Show/Hide/button>'),
 #  tags$div(id = 'DV1', class="collapse", style= "width: 440px",
...
#      
#   renderUI({
#   selectInput(inputId = "saved2",label = h4(strong("Choose scenario to display")), choices = c("Select analysis", names(database)), selected = NULL, width = "350px")
# })  
#     )
# 
#       )
...
#HTML('<button data-toggle="collapse"  data-target="#DV2" " Show/Hide/button>'),
   #tags$div(id = 'DV2', class="collapse", style= "width: 440px",
...
#      
#   renderUI({
#   selectInput(inputId = "saved3",label = h4(strong("Choose scenario to display")), choices = c("Select analysis", names(database)), selected = NULL, width = "350px")
# })  
#     )
# 
#       )
...
#HTML('<button data-toggle="collapse"  data-target="#DV3" " Show/Hide/button>'),
   #tags$div(id = 'DV3', class="collapse", style= "width: 440px",
...
#      
#   renderUI({
#   selectInput(inputId = "saved4",label = h4(strong("Choose scenario to display")), choices = c("Select analysis", names(database)), selected = NULL, width = "350px")
# })  
#     )
# 
#       )
...
#HTML('<button data-toggle="collapse"  data-target="#DV4" " Show/Hide/button>'),
   #tags$div(id = 'DV4', class="collapse", style= "width: 440px",
