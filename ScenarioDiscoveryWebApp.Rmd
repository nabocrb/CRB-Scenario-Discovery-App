...
absolutePanel(
  ...
  actionButton(inputId = "RobustnessApp",label = "Open robustness app", onclick="location.href='https://nabocrb.shinyapps.io/CRB-Robustness-App-JWRPM';", width = "110%"),
...
observeEvent( list(update$thresholds2, update$PRIMalg, update$ensemble2, update$policy2, update$AndOr2, update$metrics) ,{

  update$SDButton=ifelse(sum(update$thresholds2, update$PRIMalg, update$ensemble2, update$policy2, update$AndOr2, update$metrics)>0,
...
observeEvent(x$keep,{ # trigger *asterisks* to appear on page 2 so user knows inputs to SD have changed
  update$metrics=1
}, ignoreInit = T)
...
observeEvent( list(update$thresholds2, update$ensemble2, update$PRIMalg, update$policy2, update$AndOr2, update$metrics) ,{

  update$PTtitle=ifelse(sum(update$thresholds2, update$ensemble2, update$PRIMalg, update$policy2, update$AndOr2, update$metrics) > 0,
  "*Peeling trajectory*",
  "Peeling trajectory"
  )
...
Create & view scenarios {data-orientation=columns, style="position:relative;"}
================================
...
fluidRow(column(width=6, #style='margin-bottom:15px;margin-right: 0px; border:1px solid; padding: 10px;',
    ...
    my_step=if(str_detect(pattern = "MaxDur.",string = input$TDmetrics[1])){0.5} else{NULL}

    sliderInput("TDmetric1", label=paste0(input$TDmetrics[1], " range"),
                step = my_step,
    ...
    my_step=if(str_detect(pattern = "MaxDur.",string = input$TDmetrics[2])){0.5} else{NULL}
    ...
    my_step=if(str_detect(pattern = "MaxDur.",string = input$TDmetrics[3])){0.5} else{NULL}

    sliderInput("TDmetric3", label=paste0(input$TDmetrics[3], " range"), step = my_step,
                min=round(min(SOW_all[[input$TDmetrics[3]]]),2),
    ...
    my_step=if(str_detect(pattern = "MaxDur.",string = input$TDmetrics[4])){0.5} else{NULL}

    sliderInput("TDmetric4", label=paste0(input$TDmetrics[4], " range"), step = my_step,
                min=round(min(SOW_all[[input$TDmetrics[4]]]),2),
    ...
    my_step=if(str_detect(pattern = "MaxDur.",string = input$TDmetrics[5])){0.5} else{NULL}

    sliderInput("TDmetric5", label=paste0(input$TDmetrics[5], " range"), step = my_step,
                min=round(min(SOW_all[[input$TDmetrics[5]]]),2),
    ...
    my_step=if(str_detect(pattern = "MaxDur.",string = input$TDmetrics[6])){0.5} else{NULL}

    sliderInput("TDmetric6", label=paste0(input$TDmetrics[6], " range"), step = my_step,
                min=round(min(SOW_all[[input$TDmetrics[6]]]),2),
    ...
    my_step=if(str_detect(pattern = "MaxDur.",string = input$TDmetrics[7])){0.5} else{NULL}

    sliderInput("TDmetric7", label=paste0(input$TDmetrics[7], " range"), step = my_step,
                min=round(min(SOW_all[[input$TDmetrics[7]]]),2),
...
renderUI({
  selectizeInput(inputId = "TDpolicyID",label = "Select policy(s) to compare", choices = c(1:nrow(wide_data)),
               options = list(maxItems = ifelse(input$TDScrollFormat==T, 2, 999)), selected = NULL, multiple = T, width="90%")
}),

selectInput(inputId = "TDSOWensemble", label = "Choose SOW ensemble", choices = c("cLHS 300", "cLHS 500"), selected = "cLHS 300", width="90%"),

  prettyToggle(inputId = "DefaultVulnerability", label_on = "Use vulnerabilities from page 1",label_off = "Define vulnerability here", value = T),
...
renderUI({
  ...
  radioButtons(inputId = "ScenarioTable", label = "Scenario to display in table", choices = TDcalcs$scen_names)
    ...
    dropdownButton(circle = F, label = "Plot settings", width = "120%",
                   ...
                   prettyToggle(inputId = "TDScrollFormat", label_on = "Plot policies on same scatter plot", label_off = "Plot one scatter plot per policy", value = F),
                   
                   prettyToggle(inputId = "TDDiagonalTF",label_on = "Remove kernel density plots", label_off = "Include kernel density plots", value = F),
                   
                  sliderInput(inputId = "TDlegendx",label = "legend x position", min = 0.0, max = 1,value = 0.05, step = 0.01, width="90%", ticks = F),
                  sliderInput(inputId = "TDlegendy",label = "legend y position", min = -.5, max = .5,value = -.02, step = 0.01, width="90%", ticks = F),      
    ...
    if(input$DefaultVulnerability==F){my_selection=NULL
      } else if (isTruthy(thresholds$objectives) & input$DefaultVulnerability==T){
      my_selection=thresholds$objectives
      } else {
        my_selection=NULL
      }
      
      selectInput("TDObj",label="Select objectives", multiple = T, choices=colnames(obj_all)[-c(1,2)], selected = my_selection)
    ...
    renderUI({
      ...
      if(input$DefaultVulnerability==F){my_selection=NULL
      } else if (input$DefaultVulnerability==T){
      my_selection=input$AndOr
      } else {
        my_selection=NULL
      }
      
      radioButtons(inputId = "AndOrTD", label = "Condition for multiple objectives", choices = c("Or", "And"), inline = T, selected = my_selection)
...
# logic to render sliders with same values from Page 1 if default vulnerability thresholds are used
      if(input$DefaultVulnerability==F){ # if false, initialize with max value
        my_selection=max(obj_all[[input$TDObj[1]]])
      } else if (input$DefaultVulnerability==T){ # if true, initialize with same value as page 1
        my_selection=thresholds$thresholds[1]
      } else {
        my_selection=max(obj_all[[input$TDObj[1]]])
      }
    ...
    sliderInput("TDThresh1", label=paste0(input$TDObj[1], " threshold"), min=min(obj_all[[input$TDObj[1]]]),
                max=max(obj_all[[input$TDObj[1]]]), value=my_selection, ticks = F)
    }),
    
    renderUI({
    req(length(input$TDObj) >=2)
      
        if(input$DefaultVulnerability==F){
        my_selection=max(obj_all[[input$TDObj[2]]])
      } else if (input$DefaultVulnerability==T){
        my_selection=thresholds$thresholds[2]
      } else {
        my_selection=max(obj_all[[input$TDObj[2]]])
      }
      
    sliderInput("TDThresh2", label=paste0(input$TDObj[2], " threshold"), min=min(obj_all[[input$TDObj[2]]]),
                max=max(obj_all[[input$TDObj[2]]]), value=my_selection, ticks = F)
    }),
    
    renderUI({
    req(length(input$TDObj) >=3)
      
        if(input$DefaultVulnerability==F){
        my_selection=max(obj_all[[input$TDObj[3]]])
      } else if (input$DefaultVulnerability==T){
        my_selection=thresholds$thresholds[3]
      } else {
        my_selection=max(obj_all[[input$TDObj[3]]])
      }
      
    sliderInput("TDThresh3", label=paste0(input$TDObj[3], " threshold"), min=min(obj_all[[input$TDObj[3]]]),
                max=max(obj_all[[input$TDObj[3]]]), value=my_selection, ticks = F)
    }),
    
    renderUI({
    req(length(input$TDObj) >=4)
      
        if(input$DefaultVulnerability==F){
        my_selection=max(obj_all[[input$TDObj[4]]])
      } else if (input$DefaultVulnerability==T){
        my_selection=thresholds$thresholds[4]
      } else {
        my_selection=max(obj_all[[input$TDObj[4]]])
      }
      
    sliderInput("TDThresh4", label=paste0(input$TDObj[4], " threshold"), min=min(obj_all[[input$TDObj[4]]]),
                max=max(obj_all[[input$TDObj[4]]]), value=my_selection, ticks = F)
    }),
    
    renderUI({
    req(length(input$TDObj) >=5)
      
        if(input$DefaultVulnerability==F){
        my_selection=max(obj_all[[input$TDObj[5]]])
      } else if (input$DefaultVulnerability==T){
        my_selection=thresholds$thresholds[5]
      } else {
        my_selection=max(obj_all[[input$TDObj[5]]])
      }
      
    sliderInput("TDThresh5", label=paste0(input$TDObj[5], " threshold"), min=min(obj_all[[input$TDObj[5]]]),
                max=max(obj_all[[input$TDObj[5]]]), value=my_selection, ticks = F)
    }),

    renderUI({
    req(length(input$TDObj) >=6)
      
      if(input$DefaultVulnerability==F){
        my_selection=max(obj_all[[input$TDObj[6]]])
      } else if (input$DefaultVulnerability==T){
        my_selection=thresholds$thresholds[6]
      } else {
        my_selection=max(obj_all[[input$TDObj[6]]])
      }
    ...
    sliderInput("TDThresh6", label=paste0(input$TDObj[6], " threshold"), min=min(obj_all[[input$TDObj[6]]]),
                max=max(obj_all[[input$TDObj[6]]]), value=my_selection, ticks = F)
    }),

    renderUI({
    req(length(input$TDObj) >=7)
      
       if(input$DefaultVulnerability==F){
        my_selection=max(obj_all[[input$TDObj[7]]])
      } else if (input$DefaultVulnerability==T){
        my_selection=thresholds$thresholds[7]
      } else {
        my_selection=max(obj_all[[input$TDObj[7]]])
      }
      
    sliderInput("TDThresh7", label=paste0(input$TDObj[7], " threshold"), min=min(obj_all[[input$TDObj[7]]]),
                max=max(obj_all[[input$TDObj[7]]]), value=my_selection, ticks = F)
    }),

    renderUI({
    req(length(input$TDObj) >=8)
      
      if(input$DefaultVulnerability==F){
        my_selection=max(obj_all[[input$TDObj[8]]])
      } else if (input$DefaultVulnerability==T){
        my_selection=thresholds$thresholds[8]
      } else {
        my_selection=max(obj_all[[input$TDObj[8]]])
      }
      
    sliderInput("TDThresh8", label=paste0(input$TDObj[8], " threshold"), min=min(obj_all[[input$TDObj[8]]]),
                max=max(obj_all[[input$TDObj[8]]]), value=my_selection, ticks = F)
    }),

    renderUI({
    req(length(input$TDObj) >=9)
      
       if(input$DefaultVulnerability==F){
        my_selection=max(obj_all[[input$TDObj[9]]])
      } else if (input$DefaultVulnerability==T){
        my_selection=thresholds$thresholds[9]
      } else {
        my_selection=max(obj_all[[input$TDObj[9]]])
      }
      
    sliderInput("TDThresh9", label=paste0(input$TDObj[9], " threshold"), min=min(obj_all[[input$TDObj[9]]]),
                max=max(obj_all[[input$TDObj[9]]]), value=my_selection, ticks = F)
    }),
    ...
    renderUI({
      ...
      downloadButton(outputId = "downloadReport", icon = icon("download"), width="20%", style="margin-botom: 0px; margin-top: 0px", label="Download plots")
...
shinyjs::useShinyjs(rmd=T)
  ...
  time=if(!isTruthy(input$TDObj)){3000} else {3000+1000*(length(input$TDObj)-1)}

  if(input$DefaultVulnerability==T){

    delay(time,disable("TDObj")) # must delay the disable function such that the button renders first
    delay(time,disable("TDThresh1"))
    delay(time,disable("AndOrTD"))
    delay(time, disable("COITD"))
    
    
    req(thresholds$thresholds[2])
    delay(time,disable("TDThresh2"))
    
    req(thresholds$thresholds[3])
    delay(time,disable("TDThresh3"))
    
    req(thresholds$thresholds[4])
    delay(time,disable("TDThresh4"))
    
    req(thresholds$thresholds[5])
    delay(time,disable("TDThresh5"))
    
    req(thresholds$thresholds[6])
    delay(time,disable("TDThresh6"))
    
    req(thresholds$thresholds[7])
    delay(time,disable("TDThresh7"))
    
    req(thresholds$thresholds[8])
    delay(time,disable("TDThresh8"))
    
    req(thresholds$thresholds[9])
    delay(time,disable("TDThresh9"))
  ...
  if(input$DefaultVulnerability==F){
    ...
    delay(time,enable("TDObj"))
    delay(time,enable("TDThresh1"))
    delay(time,enable("AndOrTD"))
    delay(time, enable("COITD"))

    req(thresholds$thresholds[2])
    delay(time,enable("TDThresh2"))
    
    req(thresholds$thresholds[3])
    delay(time,enable("TDThresh3"))
    
    req(thresholds$thresholds[4])
    delay(time,enable("TDThresh4"))
    
    req(thresholds$thresholds[5])
    delay(time,enable("TDThresh5"))
    
    req(thresholds$thresholds[6])
    delay(time,enable("TDThresh6"))
    
    req(thresholds$thresholds[7])
    delay(time,enable("TDThresh7"))
    
    req(thresholds$thresholds[8])
    delay(time,enable("TDThresh8"))
    
    req(thresholds$thresholds[9])
    delay(time,enable("TDThresh9"))
...
bsTooltip(id = "TDScrollFormat", title = "WARNING!!!! By changing the plot format, the -Select policy(s) to compare- input will be cleared. If you want to switch the plot format, please remember the policy IDs and reenter them above.", placement = "right", options =list(container="body") )
...
# delay the invalidation of plots by slowing down the response time of the observable condition slider inputs

UDbox_lims_delay=reactive({
  TDcalcs$UDbox_lims
}) %>% debounce(1000)
...
SOW and scenario { style="height:500%;"}
----------------------------------

```{r, calculate COI top-down page}

TDcalcs=reactiveValues(box_list=list(), scen_names=c())
...
observeEvent(list(input$TDmetrics,input$whichBox,input$TDsaved),{ # find all metrics to plot based on user input
    ...
    # cat(file=stderr(), 'all metrics 1:', all_names, "\n") # prints dimension to R console

    if(isTruthy(input$TDsaved) & (2 %in% input$whichBox)){
      for(i in 1:length(input$TDsaved)){
        
              #cat(file=stderr(), 'saved scenario: ', input$TDsaved[i], "\n") # prints dimension to R console
        
              req(isTruthy(TDcalcs$box_list[[input$TDsaved[i]]]))
              
              #cat(file=stderr(), 'names 2 ', colnames(TDcalcs$box_list[[input$TDsaved[i]]]), "\n") # prints dimension to R console
    ...
    if(length(all_names) == 1 ){ #add a metric so you can plot 1D scenarios
        all_names[2]="Index"
      }

    all_names=unique(all_names)  
    #cat(file=stderr(), 'all metrics FINAL:', all_names, "\n") # prints dimension to R console

    TDcalcs$all_names=all_names
}, priority = -100)


################## calculate COI for selected policies, objectives, and thresholds #################

observeEvent(list(TDcalcs$objectives, TDcalcs$thresholds, input$TDpolicyID, input$TDSOWensemble, input$AndOrTD, input$DefaultVulnerability, thresholds$thresholds, thresholds$objectives, input$TDScrollFormat, input$TDDiagonalTF, TDcalcs$all_names, TDcalcs$box_list, input$ScenarioTable),{ 
  ...
  if(input$TDScrollFormat==T){# compute COI for two policies displayed on one plot
  ...
  } # end if (input$TDSCrollFormat==T)
  
  ############### if input$TDScrollFormat== F (multiple plots option) ######################

  if(input$TDScrollFormat==F){
  ...
  #cat(file=stderr(), 'n boxes:', length(box_lims), "\n") # prints dimension to R console
...
# loop through policies. Create scatter matrix and compute density, coverage
  
  for (i in 1:length(input$TDpolicyID)){
    
      objData=dplyr::filter(obj_all, ID == input$TDpolicyID[i])
      ...
      COI=defineCOI(objectives_names = TDcalcs$objectives, objectives_data = objData, thresholds=TDcalcs$thresholds, AndOrTF = AndOrTF)
      

        ####### compute density and coverage ############
        
            calcs=matrix(nrow=nrow(SOW), ncol=ndims+1)

            for(n in 1:ndims){ # loop through metrics in box, label within each box dimension range
              
              name=colnames(box)[n]
              
              calcs[,n]=(SOW[[name]] >= box[[name]][1]) & (SOW[[name]] <= box[[name]][2])

            }
            
            if(ndims>1){
              temp=rowSums(calcs[,1:ndims]) # sum of rows tells you how many dimension of SOW are in the scenario
            ...
            calcs[,ndims+1]=ifelse(temp == ndims, 1, 0) # assign 1 if within box
          
            temp=calcs[,ndims+1]+COI # if sum is 2, then vulnerable and in scenario
          
            density=sum(temp==2)/sum(calcs[,ndims+1]==1) # number of vulnerable points in box/total number of points in box
            coverage=sum(temp==2)/sum(COI)
            
            IDiVector=c(paste0(sum(COI),"/",nrow(SOW)), round(coverage,2), round(density,2))

            if(i==1){

              TDcalcs$table=data.frame(IDiVector)
              colnames(TDcalcs$table)=paste("ID:", input$TDpolicyID[1])
              row.names(TDcalcs$table)= c("total vulnerable cases", "coverage", "density")
            } else {
              TDcalcs$table=data.frame(TDcalcs$table, IDiVector)
              colnames(TDcalcs$table)=paste("ID:", input$TDpolicyID[1:i])
            }
          
        ####### end of computing density and coverage
        
      Vulnerable=ifelse(COI==1, "Yes", "No")
      TDcalcs$ensemble$Vulnerable=Vulnerable

      all_names=TDcalcs$all_names
      if(length(all_names)==1){
        all_names[2]="Index"
      }
      
      # cat(file=stderr(), 'all_names:', TDcalcs$all_names, "\n") # prints dimension to R console
      
      temp=ggpairs(data=TDcalcs$ensemble, columns = all_names,
        aes(color=Vulnerable, alpha=0.3),
        lower =  list(continuous=wrap(SD_plot2, box_lims=box_lims, npolicy=1, name_vec=TDcalcs$scen_names)),
        upper=list(continuous = "blank"), legend=NULL, switch="both", title=NULL,
        diag = diagInput)+
        theme(text=element_text(size=labelSize), axis.text.x = element_text(angle=90, size=tickSize), axis.text.y =element_text(size=tickSize))

    if(i==1){ # steal legend on first iteration
        TDcalcs$temp_TD=temp[2,1] # extract plot to steal legend from
        temp=temp+ggtitle("Vulnerable SOW")
    }  
      
  
    if( input$TDDiagonalTF==F){
        temp=gpairs_lower(temp) # remove extra labels when diagonal not wanted
    }

    plot_list[[i]]=ggmatrix_gtable(temp) # convert to gtable so plot_grid will work

      
    } # end policy loop
  
  TDcalcs$plot_list=plot_list
  

  } # end if (input$TDScrollFormat==F)
    
  
})


observeEvent(list(input$TDsaved, UDbox_lims_delay(), input$whichBox), {
...
TDcalcs$scen_names=c()
  ...
  if (1 %in% input$whichBox & isTruthy(input$TDmetric1)){ # 1 corresponds to self-defined box
    ...
    TDcalcs$box_list[[1]]=UDbox_lims_delay()
  ...
  if(2 %in% input$whichBox){ # 2 corresponds to saved boxes
    ...
    for(i in 1:length(input$TDsaved)){
      ...
      # cat(file=stderr(), 'n scenarios:', length(TDcalcs$box_list), "\n") # prints dimension to R console
...
observeEvent(list(input$ScenarioTable, input$TDpolicyID, TDcalcs$box_list, input$TDSOWensemble, input$COITD,
                  thresholds$thresholds, thresholds$objectives),{
  ...
  TDcalcs$abtable=data.frame(IDaVector)
  colnames(TDcalcs$abtable)=paste("ID:", input$TDpolicyID[1])
  row.names(TDcalcs$abtable)= c("total vulnerable cases", "coverage", "density")
  ...
  if(length(input$TDpolicyID)>1){
    ...
    tempb=rowSums(bCalcs[,1:ndims]) # sum of rows tells you how many dimension of SOW are in the scenario
      ...
      TDcalcs$abtable=cbind(IDaVector, IDbVector)
      colnames(TDcalcs$abtable)=c(paste("ID:", input$TDpolicyID[1]),paste("ID:", input$TDpolicyID[2]) )
  row.names(TDcalcs$abtable)= c("total vulnerable cases", "coverage", "density")
...
observeEvent(input$TDSOWensemble, {
      ...
      TDcalcs$ensemble$Index=1:nrow(TDcalcs$ensemble) ## arbitrary index used for plotting when only 1 observable condition is given
...
renderPlot({
  ...
  validate(need(isTruthy(input$TDmetrics[1])| isTruthy(input$TDsaved), "Select at least one observable condition or select a saved scenario to plot."))
  ...
  validate(need( (isTruthy(TDcalcs$IDaCOI) | isTruthy(TDcalcs$plot_list) ), "Define vulnerability on page 1 or to the left."))
  ...
  if(input$TDScrollFormat==T){ # plot policies on same scatter plot
  ...
  pairs_TD=pairs_TD+theme(axis.text.x = element_text(angle=90, size=tickSize), axis.text.y =element_text(size=tickSize))
    ...
    # if(length(TDcalcs$all_names)>2){
    #   my_height=my_height*length(TDcalcs$all_names) # make additional space for extra metrics
    # }
    # 
    # if(input$TDDiagonalTF==T ){
    #    my_height=my_height+4 # add space for diagonal
    # }
        
    vp_main=viewport(width=unit(x=1, units = "npc"), height =unit(x=my_height, units = "npc"), x = 0, y=1, just = c(0,1))
    vp_TD=viewport(width = 1, height=2, x=x, y=y, just = c(.45,0)) # viewport for COI
    vp_scenLegend=viewport(width=1, height=2, x=x,y=y-.022, just=  c(.45,0)) # viewport for scenario names
  
    # save scatter plot
    TD$scatter=pairs_TD
    TD$scatterVP=vp_main

    # Print plots
    print(pairs_TD, vp=vp_main)
    ...
    #############################################################################################
    ######### plot each policy in its own scatter plot, growing in the y direction ##############
  ...
  if(input$TDScrollFormat==F){ 
    ...
    temp_TD=TDcalcs$temp_TD #plot to steal legend from
    ...
    my_height=4*length(TDcalcs$plot_list) #calculate height of plot. Allocate four inches per plot
    ...
    vp_main=viewport(width=unit(x=1, units = "npc"), height =unit(x=my_height, units = "inches"), x = 0, y=1, just = c(0,1))
    vp_TD=viewport(width = 1, height=2, x=x, y=y, just =c(.45,0)) # viewport for COI
    vp_scenLegend=viewport(width=1, height=2, x=x,y=y-.015, just=c(.45,0)) # viewport for scenario names
    
    column_plot=plot_grid(plotlist=TDcalcs$plot_list, ncol=1, align="v", labels = as.character(input$TDpolicyID), vjust=c(1, rep(0.5, length(TDcalcs$plot_list)-1)), label_size = 16)
    
    # Save scatter plot
    TD$scatter=column_plot
    TD$scatterVP=vp_main
    
    # Print plots
    print(column_plot, vp=vp_main)
    print(legend_TD, vp=vp_TD)
    print(scen_legend, vp=vp_scenLegend)
  ...
  TD$pointLegend=legend_TD
  TD$pointVP=vp_TD
  TD$boxLegend=scen_legend
  TD$boxVP=vp_scenLegend
...
absolutePanel(class = "panel panel-default", fixed = T,
   ...
   tags$div(id = 'boxTD1', class="collapse in",
      ...
      renderTable({
        ...
        if(input$TDScrollFormat==T){ # plot policies on same scatter plot
          TDcalcs$abtable
        } else { # plot policies in separate scatter plots
          TDcalcs$table
        }
...
```{r, download report, include=FALSE}

output$downloadReport <- downloadHandler(
    filename = function() { "Create&ViewScenarios.pdf" },
    content = function(file) {
      
      # open device
      
      pdf(file, width = 10, height = TD$height )
      
        # create plot
      
        print(TD$scatter)
        
        # legend position is way off
        
        x=input$TDlegendx+.1
        y=input$TDlegendy*5+.3
        vp_points=viewport(width = unit(x=1, units = "npc"), height=unit(x=1, units = "npc"), x=unit(x=.8, units="npc"), y=unit(x=.8, units="npc"), just =c(0,0)) # viewport for COI
        vp_scenLegend=viewport(width=unit(x=1, units = "npc"), height=unit(x=1, units = "npc"), x=unit(.8, units="npc"),y=unit(x=.8, units="npc")-unit(x=1, units="inches"), just=c(0,0)) # viewport for scenario names

        print(TD$pointLegend, vp=vp_points)
        print(TD$boxLegend, vp=vp_scenLegend)

        # close the device
      
        dev.off()
        
        #ggsave(file, plot = TD$plot, device = "pdf", dpi=400)
    }
)
